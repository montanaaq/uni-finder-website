...
import { useState, useRef, FC, SetStateAction } from 'react'
...
const MainContainer: FC<MainContainerProps> = ({ selectedCity }) => {
  const [firstColumn, setFirstColumn]: [ColumnType[], SetStateAction<object>] =
    useState([
      ...
      { id: 2, subjectName: 'Русский язык', score: '' },
      { id: 3, subjectName: 'Информатика', score: '' },
      { id: 4, subjectName: 'Физика', score: '' },
      { id: 5, subjectName: 'Литература', score: '' }
    ])
  const [secondColumn, setSecondColumn]: [
    ...
    { id: 9, subjectName: 'Обществознание', score: '' },
  ...
  const [thirdColumn, setThirdColumn]: [ColumnType[], SetStateAction<object>] =
    useState([
      ...
      { id: 15, subjectName: 'Немецкий язык', score: '' }
    ])
  ...
  const joinSubjects = (
    ...
    return subject
      .map(subj_id => `${subj_id.subjectName} - ${subj_id.score} баллов\n`)
      .join(',')
  }
  const completed_msg: string = `Куда я могу поступить в городе ${selectedCity}\nМои сданные предметы: ${joinSubjects(
    firstColumn,
    secondColumn,
    thirdColumn
  )}\nТакже выдай ссылку на все предложенные университеты`
